...
def get_current_dislocation() -> List:
    ...
    locations = []
    arrivale_dates = [
        None, None, None, datetime.now() - timedelta(days=3), datetime.now()
    ]
    time.sleep(2)
...
def api_call():
    ...
    for i, ind in enumerate(index):
        if locations[ind].get('invoice') == predicted_data[i].get('invoice'):
            locations[ind]['arrivale_date'] = predicted_data[i].get('predicted_date')
